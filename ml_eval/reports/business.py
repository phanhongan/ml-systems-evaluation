...
class BusinessImpactReport(BaseReport):
    """Generate business impact reports connecting technical metrics to business outcomes"""
    ...
    def generate(self, data: Dict[str, Any]) -> ReportData:
        ...
        recommendations = []
        if financial_loss > 0:
            recommendations.append(f"Financial loss of ${financial_loss:,.2f} detected. Review system reliability.")
        if downtime_hours > 1:
            recommendations.append(f"{downtime_hours} hours of downtime. Implement redundancy.")
        if customer_impact < 0.8:
            recommendations.append("Customer impact score below 80%. Improve system performance.")
        if total_incidents > 5:
            recommendations.append(f"{total_incidents} incidents detected. Review incident response.")
            
        return ReportData(
            title="Business Impact Report",
            generated_at=datetime.now(),
            period=data.get("period", "30d"),
            metrics={"business_metrics": business_metrics, "technical_metrics": technical_metrics, "financial_impact": financial_impact},
            summary=summary,
    ...
    def format_report(self, report_data: ReportData) -> str:
        ...
        if report_data.metrics.get("business_metrics"):
            ...
            report.append("-" * 20)
            for metric_name, metric_value in report_data.metrics["business_metrics"].items():
        ...
        if report_data.metrics.get("technical_metrics") and report_data.metrics.get("business_metrics"):
            ...
            if "availability" in tech_metrics and "downtime_hours" in biz_metrics:
                ...
                report.append(f"System Availability: {availability:.2%} → Downtime: {downtime:.1f} hours")
            ...
            if "error_rate" in tech_metrics and "customer_impact_score" in biz_metrics:
                ...
                report.append(f"Error Rate: {error_rate:.2%} → Customer Impact: {customer_impact:.1%}")
            report.append("")
