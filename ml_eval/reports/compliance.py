...
class ComplianceReport(BaseReport):
    ...
    def generate(self, data: Dict[str, Any]) -> ReportData:
        ...
        compliant_standards = sum(1 for standard in regulatory_standards if compliance_metrics.get(standard, {}).get("compliant", False))
        ...
        if compliance_rate < 1.0:
            recommendations.append("Not all regulatory standards are compliant. Review required.")
        if not audit_trail:
        ...
        return ReportData(
            ...
            metrics={"compliance_metrics": compliance_metrics, "audit_trail": audit_trail, "standards": regulatory_standards},
            summary=summary,
    ...
    def format_report(self, report_data: ReportData) -> str:
        ...
        if report_data.metrics.get("compliance_metrics"):
            ...
            for standard, metrics in report_data.metrics["compliance_metrics"].items():
                status = "✓ COMPLIANT" if metrics.get("compliant", False) else "✗ NON-COMPLIANT"
        ...
        if report_data.metrics.get("audit_trail"):
            ...
            for entry in report_data.metrics["audit_trail"][-5:]:  # Show last 5 entries
                report.append(f"[{entry.get('timestamp', 'Unknown')}] {entry.get('action', 'Unknown action')}")
