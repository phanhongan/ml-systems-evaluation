...
class ReliabilityReport(BaseReport):
    ...
    def generate(self, data: Dict[str, Any]) -> ReportData:
        ...
        exhausted_budgets = sum(1 for budget in error_budgets.values() if budget.get("exhausted", False))
        ...
        if summary["compliance_rate"] < 0.95:
            recommendations.append("SLO compliance rate is below 95%. Review system performance.")
        if exhausted_budgets > 0:
            recommendations.append(f"{exhausted_budgets} error budgets exhausted. Immediate action required.")
        if len(incidents) > 0:
            recommendations.append(f"{len(incidents)} incidents detected. Review incident response procedures.")
            
        return ReportData(
            title="Reliability Report",
            generated_at=datetime.now(),
            period=data.get("period", "24h"),
            metrics={"slos": slos, "error_budgets": error_budgets, "incidents": incidents},
            summary=summary,
    ...
    def format_report(self, report_data: ReportData) -> str:
        ...
        if report_data.metrics.get("slos"):
            ...
            for slo_name, slo_data in report_data.metrics["slos"].items():
                ...
                report.append(f"{status} {slo_name}: {slo_data.get('current_value', 'N/A')} / {slo_data.get('target', 'N/A')}")
            report.append("")
        ...
        if report_data.metrics.get("error_budgets"):
            ...
            report.append("-" * 20)
            for budget_name, budget_data in report_data.metrics["error_budgets"].items():
                status = "EXHAUSTED" if budget_data.get("exhausted", False) else "OK"
                report.append(f"{budget_name}: {budget_data.get('remaining', 'N/A')} remaining ({status})")
            report.append("")
